...
class PortraitWindow(QWidget):
    def __init__(self, original_resolution, config_path, parent=None):
        ...
        self.finished_button.setStyleSheet("background-color: green; color: white; font-weight: bold;")
...
class DrawWidget(QWidget):
    ...
    def get_handle_at(self, pos):
        ...
        # Edges (checking strict bounds on the perpendicular axis)
...
class CropApp(QWidget):
    def __init__(self, file_path=None):
        ...
        self.media = None
        self.snapshot_path = os.path.join(os.getcwd(), "snapshot.png")
        for ext in [".png", ".jpg", ".jpeg"]:
            garbage_path = os.path.join(os.getcwd(), f"snapshot{ext}")
        ...
        self.send_crop_button.setStyleSheet("background-color: #e67e22; color: white;")
        ...
        control_layout.setSpacing(20) # 20px gap between buttons
        control_layout.addStretch()   # Push to center (Left spacer)
        control_layout.addWidget(self.open_button)
        control_layout.addWidget(self.play_pause_button)
        control_layout.addWidget(self.snapshot_button)
        control_layout.addWidget(self.back_button)
        control_layout.addWidget(self.send_crop_button)
        control_layout.addStretch()   # Push to center (Right spacer)
        
        vbox = QVBoxLayout()
        vbox.addWidget(self.view_stack, 1)
        vbox.addWidget(self.position_slider)
        vbox.addLayout(control_layout)
        vbox.addWidget(self.coordinates_label) # Label moved directly to main layout
        ...
        # Call set_style AFTER creating buttons
        self.set_style()
    ...
    def set_style(self):
        # Increased max-width to 75px (approx 20% increase from 60px)
        self.setStyleSheet(
            """
        ...
        """)
        # Increased width for the longer text button
    ...
    def take_snapshot(self):
        ...
        if not self.original_resolution:
            print("Error: No resolution info.")
            self.coordinates_label.setText("Please wait for video information to be loaded.")
            return
        ...
        print(f"Attempting snapshot to: {self.snapshot_path}")
        result = self.media_player.video_take_snapshot(0, self.snapshot_path, 0, 0)
        
        QTimer.singleShot(300, self._show_draw_view)
    ...
    def _show_draw_view(self):
        if os.path.exists(self.snapshot_path):
            ...
            self.coordinates_label.setText("Failed to take snapshot (File missing).")
