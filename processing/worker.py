    ...
    def run(self):
            ...
            try:
                ...
                def run_ffmpeg_command(encoder_name):
                    ...
                    filters = []
                    filters.append(f"{granular_video_label}{v_filter_cmd}")
                    if not self.speed_segments and self.speed_factor != 1.0:
                        filters.append(f"setpts=PTS/{self.speed_factor}")
                    if not self.disable_fades:
                         ...
                         if vfade_in_sec > 0:
                             filters.append(f"fade=t=in:st=0:d={vfade_in_sec:.4f}")
                         if vfade_out_sec > 0:
                             filters.append(f"fade=t=out:st={vfade_out_start_sec:.4f}:d={vfade_out_sec:.4f}")
                    filters.append("format=nv12")
                    filters.append(f"trim=duration={self.duration_corrected_sec:.6f}")
                    filters.append("setpts=PTS-STARTPTS")
                    filters.append("setsar=1")
                    vcore_str = ",".join(filters)
                    attempt_core_filters.append(f"{vcore_str}[vcore]")
                ...
                if not success and not self.is_canceled:
                    self.finished_signal.emit(False, "Main video rendering failed after all attempts.")
                    return
                if self.is_canceled: return
