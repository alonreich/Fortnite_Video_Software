...
class ProcessThread(QThread):
    def __init__(self, input_path, start_time, end_time, original_duration, original_resolution, progress_update_signal, status_update_signal, finished_signal, script_dir, is_mobile_format, speed_factor):
        super().__init__()
    ...
    def get_total_frames(self):
        """
        Uses ffprobe to get the total number of frames in the trimmed video section.
        """
        ffprobe_path = os.path.join(self.script_dir, 'ffprobe.exe')
        ...
        try:
            ...
            # The number of frames might be reported on the stream or format level,
            # so we check both.
    ...
    def run(self):
        ...
        """
        temp_log_path = os.path.join(self.script_dir, f"ffmpeg2pass-{os.getpid()}-{int(time.time())}.log")
        ...
        try:
            ...
            if self.speed_factor != 1.0:
                ...
                self.status_update_signal.emit(f"Adjusting trim times for speed factor {self.speed_factor}x. Prepare for a cinematic masterpiece.")
            else:
                start_time_corrected = self.start_time
                end_time_corrected = self.end_time
                duration_corrected = self.end_time - self.start_time
            
            # Use the corrected values for the rest of the calculations
            ...
            try:
                ...
                if video_bits < 0:
                    self.finished_signal.emit(False, "Video duration is too short for the target file size. You can't fit a gallon of awesome in a thimble.")
                    return
                video_bitrate_kbps = video_bits / (1024 * duration_corrected)
            except ZeroDivisionError:
                self.finished_signal.emit(False, "Selected video duration is zero. It's a paradox!")
                return
            
            self.status_update_signal.emit(f"Calculated target bitrate: {video_bitrate_kbps:.2f} kbps. It's science!")

            # Step 3: Get total frames for progress bar
            total_frames = self.get_total_frames()
            if total_frames is None:
                self.status_update_signal.emit("Could not determine total frames. Progress bar might be jerky. My apologies, I'm only human... ish.")
            
            # Step 4: Build FFmpeg command arguments
            video_filter_cmd = ""
            ...
            if self.is_mobile_format:
                ...
                self.status_update_signal.emit("Optimizing for mobile: Applying complex video filter for split-screen and UI overlays.")
            else:
                ...
                if video_bitrate_kbps < 800 and original_height > 720:
                    ...
                    self.status_update_signal.emit("Note: Low bitrate detected. Scaling to a lower HD resolution (720p) for better quality.")
            ...
            # Step 5: Execute Pass 1
            self.status_update_signal.emit("Processing... Pass 1 of 2: Analyzing video stream. The computer is thinking really, really hard.")
            ...
            pass1_cmd = [
                ...
                '-loglevel', 'info' # We need this to get the frame progress
            ]
            ...
            proc1 = subprocess.Popen(pass1_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            frame_regex = re.compile(r'frame=\s*(\d+)')
            
            for line in proc1.stdout:
                if "error" in line.lower():
                    proc1.kill()
                    raise subprocess.CalledProcessError(proc1.returncode, proc1.args, output=line)
            ...
            # Step 6: Execute Pass 2
            self.status_update_signal.emit("Processing... Pass 2 of 2: Encoding the video with final settings. This is where the magic happens.")
            ...
            pass2_cmd = [
                ...
                '-passlogfile', temp_log_path,
                '-loglevel', 'info' # We need this for progress updates
            ]
            ...
            proc2 = subprocess.Popen(pass2_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            
            for line in proc2.stdout:
                if "error" in line.lower():
                    proc2.kill()
                    raise subprocess.CalledProcessError(proc2.returncode, proc2.args, output=line)
            ...
            if proc2.returncode != 0:
                raise subprocess.CalledProcessError(proc2.returncode, proc2.args, output=proc2.stdout.read())
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                self.finished_signal.emit(False, "Output file was created, but it's empty. Did FFmpeg die for no reason? You'd have to ask it yourself.")

        except FileNotFoundError:
            self.finished_signal.emit(False, f"FFmpeg not found at {ffmpeg_path}. Seriously? It's like you're not even trying.")
        except subprocess.CalledProcessError as e:
            error_message = e.stdout or e.stderr
            if not error_message:
                error_message = "FFmpeg crashed without a detailed error. Probably something you did. The full command was printed to your console."
            self.finished_signal.emit(False, f"An FFmpeg error occurred: {error_message}")
        except Exception as e:
            self.finished_signal.emit(False, f"An unexpected error occurred: {e}. It's always something.")
        finally:
