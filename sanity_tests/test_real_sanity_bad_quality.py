...
def verify_video_movement(video_path):
    ...
    out_sheet = "movement_sheet_bad.bmp"
    cmd = [
...
def test_real_video_bad_quality():
    vid = find_test_video()
    if not vid:
        pytest.skip("No real video found in user directories to test.")
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    logger = setup_logger(base_dir, "sanity_bad.log", "SanityBAD")
    ...
    assert 8.0 <= size_mb <= 22.0, f"Output file size ({size_mb:.2f}MB) is out of range for BAD quality target 15MB (+/- 7MB)!"
