...
class EncoderManager:
    ...
    H264_LEVEL_42_MAX_KBPS = 50000

    def _vbv_buf_kbps(self, kbps: int) -> int:
        return min(self.H264_LEVEL_42_MAX_KBPS, max(kbps, kbps * 2))
        ...
        if hardware_strategy:
            ...
            if requested_encoder:
                ...
                if self.available_encoders and requested_encoder not in self.available_encoders:
                    self._encoder_preflight_error = (
                        f"Export is blocked: hardware mode is {hardware_strategy} ({requested_encoder}), but this FFmpeg build does not list that encoder. "
                        "Use a GPU-enabled FFmpeg in binaries/, or clear cached GPU mode so the app falls back to CPU."
                    )
                ...
                if hardware_strategy != "CPU":
                    self.logger.error(f"Unknown hardware strategy '{hardware_strategy}'. Refusing implicit CPU fallback.")
                    self.primary_encoder = os.environ.get('VIDEO_HW_ENCODER', 'h264_nvenc')
                    self.forced_cpu = False
                else:
                    self.primary_encoder = "libx264"
    ...
    def get_fallback_list(self, failed_encoder: str, allow_cpu: bool = True) -> list[str]:
        ...
        if str(self.hardware_strategy or "").upper() in self.HARDWARE_BY_STRATEGY:
            self.logger.error(f"GPU encoder '{failed_encoder}' failed under {self.hardware_strategy} mode. Strict GPU mode blocks fallback.")
            return []
        try:
        ...
        for i in range(start_index, len(self.ENCODER_PREFERENCE)):
            ...
            if not allow_cpu and encoder == "libx264":
                continue
            if encoder != "libx264" and self.available_encoders and encoder not in self.available_encoders:
                continue
    ...
    def get_codec_flags(self, encoder_name: str, video_bitrate_kbps: Optional[int], effective_duration_sec: float, fps_expr: str = "60000/1001", quality_level: int = 2, size_locked: bool = True) -> tuple[list[str], str]:
        ...
        if encoder_name == "libx264" and strict_gpu and not self.forced_cpu:
            raise RuntimeError("CPU encoder requested while strict GPU mode is active.")
        if self.forced_cpu:
            cpu_preset = 'fast' if quality_level <= 1 else 'medium'
            flags = ['-c:v', 'libx264', '-preset', cpu_preset, '-pix_fmt', 'yuv420p', '-profile:v', 'high', '-level:v', '4.2', '-bf', '2']
            ...
            kbps = min(self.H264_LEVEL_42_MAX_KBPS, max(300, int(video_bitrate_kbps)))
            flags.extend(['-b:v', f'{kbps}k', '-maxrate', f'{kbps}k', '-bufsize', f'{self._vbv_buf_kbps(kbps)}k'])
            self.logger.info(f"Encoder: CPU Force Enabled. Preset={cpu_preset} Bitrate={kbps}k")
        ...
        if encoder_name == 'h264_nvenc':
            if quality_level <= 0:
                nv_preset, multipass, lookahead, aq_strength = 'p4', 'qres', '24', '7'
            elif quality_level <= 1:
                nv_preset, multipass, lookahead, aq_strength = 'p5', 'qres', '32', '8'
            else:
                nv_preset, multipass, lookahead, aq_strength = 'p6', 'fullres', '48', '9'
            ...
            if video_bitrate_kbps:
                ...
                vcodec.extend([
                    '-b:v', f'{kbps}k',
                    '-maxrate', f'{kbps}k',
                    '-bufsize', f'{self._vbv_buf_kbps(kbps)}k'
                ])
                if size_locked:
                    vcodec.extend(['-cbr', '1', '-cbr_padding', '1'])
            ...
            if video_bitrate_kbps is None:
                ...
                vcodec.extend(['-preset', cpu_preset, '-crf', crf, '-bf', '2'])
